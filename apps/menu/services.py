...
class MenuService:
    ...
    @staticmethod
    def add_dish(event_id, tenant_id: str, validated_data: dict) -> EventMenuItem:
        """
        Creates an EventMenuItem for the given event (tenant-scoped).
        Raises ValidationError if the event menu is locked.
        Snapshot fields are frozen automatically inside EventMenuItem.save().
        """
        event = get_object_or_404(Event, pk=event_id, tenant_id=tenant_id)
        ...
        return EventMenuItem.objects.create(event=event, tenant_id=tenant_id, **validated_data)
